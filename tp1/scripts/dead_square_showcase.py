...
Genera una figura con:
  - tablero especial con dead squares marcados
...
BOARD_PATH = ROOT / "boards" / "sokoban" / "dead_square_corner_showcase.txt"
...
def run_case(board_path: Path) -> list[RunData]:
    return run_case_n(board_path, runs=1)


def run_case_n(board_path: Path, runs: int) -> list[RunData]:
    rows: list[RunData] = []
    for run_idx in range(1, runs + 1):
        for heuristic in ["manhattan", "manhattan+dead_square"]:
            ...
            if proc.is_alive():
                proc.terminate()
                proc.join()
                raise RuntimeError(f"Timeout inesperado para {heuristic}")
...
def summarize_rows(rows: list[RunData]) -> dict[str, dict[str, float]]:
    summary: dict[str, dict[str, float]] = {}
    for heuristic in ["manhattan", "manhattan+dead_square"]:
        subset = [row for row in rows if row["heuristic"] == heuristic]
...
def _draw_board(ax: Axes, board_path: Path) -> None:
    ...
    ax.set_title("Mapa con esquinas exageradas", fontsize=14, weight="bold", pad=12)


def build_plot(rows: list[RunData], board_path: Path, outpath: Path) -> Path:
    ...
    node_values = [
        by_name["manhattan"]["nodes_mean"],
        by_name["manhattan+dead_square"]["nodes_mean"],
    ]
    ...
    ax_nodes.set_ylim(min(node_values) * 0.92, max(node_values) * 1.12)

    ax_time = fig.add_subplot(gs[0, 2])
    time_values = [
        by_name["manhattan"]["time_mean"],
        by_name["manhattan+dead_square"]["time_mean"],
    ]
    time_stds = [
        by_name["manhattan"]["time_std"],
        by_name["manhattan+dead_square"]["time_std"],
    ]
    ...
    ax_time.set_ylim(min(time_values) * 0.92, max(time_values) * 1.12)
    ...
    outpath.parent.mkdir(parents=True, exist_ok=True)
    fig.subplots_adjust(left=0.04, right=0.98, top=0.88, bottom=0.16, wspace=0.32)
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=BOARD_PATH,
        help="Tablero especial para la comparativa",
    )
    parser.add_argument(
        "--csv",
        type=Path,
    ...
    rows = run_case_n(args.board, args.runs)
    csv_path = write_csv(rows, args.csv)
    plot_path = build_plot(rows, args.board, args.plot)
