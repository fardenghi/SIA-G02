...
def plot_recovery_rate_vs_noise(
    ...
    good_colors = {name: palette[i % len(palette)] for i, name in enumerate(good)}
    ...
    for name in rest:
        ...
        rest_plotted = True
    for name in good:
        ax.plot(noise_levels, rates[name], marker="o", label=name, linewidth=2, color=good_colors[name])
...
def plot_pattern_grid(
    ...
    """Grilla de patrones."""
    n = len(patterns)
    cols = cols if cols is not None else min(n, 8)
    rows = (n + cols - 1) // cols
    ...
    for ax, (label, pat) in zip(axes_flat, patterns):
        ax.axis("on")
        if black_and_white:
            img = np.where(pat.reshape(grid, grid) == 1, 1.0, 0.0)
