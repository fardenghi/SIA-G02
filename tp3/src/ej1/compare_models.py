...
from src.ej1.evaluation import evaluate, plot_model_comparison, print_metrics
...
def run():
    ...
    nonlinear_t = FraudTrainer.load(_OUT_DIR, "nonlinear")

    lin_mse,  lin_mae  = evaluate(linear_t.perceptron,    X_norm, y)
    sig_mse,  sig_mae  = evaluate(nonlinear_t.perceptron, X_norm, y)
    ...
    sig_h  = nonlinear_t.train_loss_history
    ...
    plot_model_comparison(
        ...
        save_path=f"{_PLOTS_DIR}/phase5_comparison.png",
    )

    # ── Guardar config del modelo elegido para Fase 6 ─────────────────────────
    selected_cfg_path = Path(_OUT_DIR) / "selected_config.json"
    with open(_OUT_DIR + "/nonlinear_config.json") as f:
        selected_cfg = json.load(f)
    selected_cfg["selected_model"] = "nonlinear"
