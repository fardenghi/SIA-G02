"""Fase 6b — Estudio de generalización: relu con K-Fold CV.
...
_MODEL_NAME = "generalization_relu"

with open(_OUT_DIR / "relu_config.json") as f:
...
def run():
    ...
    print(f"Iniciando K-Fold Cross-Validation para ReLU (K={CFG['k_folds']})")
    print("-" * 45)
    ...
    print("\nEntrenando modelo final ReLU con 100% de los datos...")
    ...
    plot_loss_curves(
        all_train_loss,
        all_val_loss,
        title=f"Generalización ReLU (K-Fold K={CFG['k_folds']})",
        save_path=_PLOTS_DIR / "generalization_loss_relu.png",
    )

    plot_predictions_distribution(
        oof_preds,
        y,
        title="Distribución predicciones ReLU (Out-Of-Fold)",
        save_path=_PLOTS_DIR / "generalization_pred_dist_relu.png",
    )

    final_trainer.save(_OUT_DIR, _MODEL_NAME)
    np.savez(_OUT_DIR / "oof_predictions_relu.npz", preds=oof_preds, labels=labels)
    ...
    print(f"\nModelo final ReLU guardado en {_OUT_DIR}/{_MODEL_NAME}.*")
    print(f"Predicciones OOF ReLU guardadas en {_OUT_DIR}/oof_predictions_relu.npz")
