...
- Las decisiones se reflejarán en `exercises/ej1_fraud/config.py`
...
Todo el código específico del Ej1 irá en `exercises/ej1_fraud/`.

### Estructura de `exercises/ej1_fraud/`
...
### Output esperado
- Directorio `exercises/ej1_fraud/` creado
...
### Contexto
Usamos `SimplePerceptron` de `common/simple_perceptron.py` con `activation=linear`.
...
### Contexto
Usamos `SimplePerceptron` de `common/simple_perceptron.py` con `activation=sigmoid`.

### Tareas

1. **Seleccionar función de activación**
   - Candidatos: `tanh`, `sigmoid`, `ReLU`
   - Sugerencia: **`sigmoid`** (está en `common/activations.py` y su rango coincide con el target [0, 1])
...
3. **Crear y entrenar modelo no lineal**
   - `perceptron_sigmoid = SimplePerceptron(input_size=10, activation=sigmoid, ...)`
   - Configuración: mismo learning_rate y epochs que lineal (para comparación justa)
   - Entrenar con `perceptron_sigmoid.train(X, y)`
...
- Reutilizar `SimplePerceptron` de `common/` (ya está probada)
- Todo código de Ej1 en `exercises/ej1_fraud/`
- Gráficas y análisis en `exercises/ej1_fraud/evaluation.py`
- Modelos y outputs generados en `outputs/ej1_fraud/`
